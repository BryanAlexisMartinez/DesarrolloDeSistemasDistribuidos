...
public class WebServer {
    private static final String TASK_ENDPOINT = "/task"; //POST
    private static final String STATUS_ENDPOINT = "/status"; //GET
    private static final String SEARCH_TOKEN = "/search"; //GET
    ...
    private void handleSearchTokenRequest(HttpExchange exchange) throws IOException {
        ...
        if (isDebugMode) {
            ...
            String debugMessage = String.format("La operacion tomo %d nanosegundos = a %d segundos con %d milisegundos", finishTime - startTime,segundos,milisegundos);
    ...
    private byte[] searchToken(byte[] requestBytes){
        ...
        while (i<n*4){
            int random = (int)(Math.random()*26 + 65);//Obtiene un valor entre los valores ASCII de A a Z
            letra = (char)random;
            if(i%4 == 3){
                cadenagrande.append(" ");
            }
        ...
        return String.format("Se encontraron %d de la cadena %s\n", contador,cadena).getBytes();
    }
